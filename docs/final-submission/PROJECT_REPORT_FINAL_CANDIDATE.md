...
### Project Report (Final Candidate)

> Polished from `PROJECT_REPORT_DRAFT.md`. Citation labels `[n]` refer to
> `REFERENCES.md`. Items still lacking a verified source are marked
> `[citation pending]` and tracked in `CITATION_GAP_LIST.md`. All technical claims
> reflect the actual repository (migrations `0001`–`0006`, the `src/` codebase,
> and the Playwright E2E suite). No detection-model benchmark numbers are claimed.
...
An accurate inventory of road traffic signs supports road-safety analysis, maintenance
planning, and asset management `[citation pending]`. Building such an inventory manually
is slow and error-prone. Automatic traffic-sign detection and recognition from imagery
have therefore been studied extensively [1], [2], and modern object detectors — for
example YOLO [3], Faster R-CNN [4], and SSD [5] — can localise and classify signs in
images. However, a raw stream of per-frame detections is not itself an inventory: the
same physical sign is detected repeatedly, GPS readings vary, and confidence fluctuates.
This project addresses the gap between *detection* and a *usable inventory* by building
an end-to-end web system that ingests detections, fuses them with geolocation, removes
duplicates, and presents a verified, queryable map.
...
workflow — is a recognised practice `[citation pending]`. A detailed comparative survey
...
fixes when computing a sign's coordinates. GPS measurements are inherently uncertain in
mobile data collection `[citation pending]`, which motivates weighting by reported
accuracy rather than treating all fixes equally.
...
implements human-in-the-loop review of the model's outputs `[citation pending]`. All
administrative APIs return HTTP 401 for unauthenticated callers and HTTP 403 for
...
- demo data seeded successfully (in a representative run: 4 devices, 6 sessions, 120
  detection events, 35 signs, and 7 daily snapshots — these are demo-dataset counts, not
...
The accuracy of the detection model is not evaluated in this work because the model is
an external service and no labelled evaluation dataset was measured here; benchmark
datasets such as [1] and [2] would support such an evaluation in future work.
...
See `REFERENCES.md` for the full bibliography ([1]–[13]). Replace any remaining
`[citation pending]` markers (tracked in `CITATION_GAP_LIST.md`) before final submission,
and confirm all entries against the required citation style.
